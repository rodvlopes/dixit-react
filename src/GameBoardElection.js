...
import { Grid, Box, Button, Collapse } from "@material-ui/core";
...
function GameBoardElectionPresentational({
  ...
  const noVoteListeners = listeners.filter(
    (p) => !cards.find((c) => c.votes.map((v) => v.index).includes(p.index))
  );
...
const electionRequiredChecker = ({
  ...
  const isAllCardsSelected =
    selectedCards.length === players.filter((p) => p.name).length;

  const isLoggedInUserTheStoryTeller = loggedInUser.index === storyTeller.index;

  const hasLoggedInUserVoted = selectedCards.filter((c) =>
    c.votes.find((v) => v.index === loggedInUser.index)
  ).length;

  return isLoggedInUserTheStoryTeller
    ? false
    : isAllCardsSelected && !hasLoggedInUserVoted;
};
